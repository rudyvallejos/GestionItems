...
class WidgetRepeater(Widget):
    ...
    params = {
        'repetitions': 'Minimum number of repetitions',
        'extra': 'Number of extra repetitions'
    }
    widget = None
    repetitions = 3
    ...
    def update_params(self, d):
        ...
        if d.get('max_repetitions', self.max_repetitions) is not None:
            warn("max_repetitions is deprecated and no longer has any effect",
                DeprecationWarning, 3)
        v_f, a_f = d['value_for'], d['args_for']
        num = len(d['value'] or [])
        outputs = [w.render(v_f(w), isextra=w.repetition >= num, **a_f(w))
            for w in d['children'].__iter__(
                max(d['repetitions'], num + d.extra))]
...
class RepeatedWidget(Widget):
    ...
    params = {
        'repetition': 'The repetition number of the widget'
    }
    repetition = 0
